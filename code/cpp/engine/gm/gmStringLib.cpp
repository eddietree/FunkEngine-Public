...
static int GM_CDECL gmfStringLower(gmThread * a_thread)
{
  ...
  for( int i = 0; i < length; ++i ) buffer[i] = tolower(buffer[i]);
#else
  strlwr(buffer);
#endif
...
#if defined(__APPLE__)
  for( int i = 0; i < length; ++i ) buffer[i] = toupper(buffer[i]);
#else
  strupr(buffer);
#endif
...
static int GM_CDECL gmStringLowerCaseToCamelCase(gmThread * a_thread)
{
  const gmVariable * var = a_thread->GetThis();
  GM_ASSERT(var->m_type == GM_STRING);
  gmStringObject * strObj = (gmStringObject *) GM_OBJECT(var->m_value.m_ref);
...
static int GM_CDECL gmStringCamelCaseToLowerCase(gmThread * a_thread)
{
  const gmVariable * var = a_thread->GetThis();
  GM_ASSERT(var->m_type == GM_STRING);
  gmStringObject * strObj = (gmStringObject *) GM_OBJECT(var->m_value.m_ref);
  const char * str = (const char *) *strObj;
  const int strLength = strObj->GetLength();
  const int maxPossibleLength = strLength * 2;

  char * buffer = (char *) alloca(maxPossibleLength + 1); 
  memset(buffer, 0, maxPossibleLength + 1);
  ...
  for ( int i = 1; i < strLength - 1; ++i )
  {
    ...
    buffer[ dst_index++ ] = tolower( c );

    if ( !isupper( c ) && isupper( next ) )
    {
      buffer[ dst_index++ ] = '_';
    }
  }

  buffer[ dst_index ] = str[ strLength - 1 ];
...
extern int GM_CDECL gmfToV2(gmThread * a_thread);
extern int GM_CDECL gmfToV2i(gmThread * a_thread);
...
{ 
  ...
  */
  ...
  /*gm
    \function LowerCaseToCamelCase
    \brief Convert string from lower_case style to CamelCase
    \return string
  */
  {"LowerCaseToCamelCase", gmStringLowerCaseToCamelCase},
  /*gm
    \function LowerCaseToCamelCase
    \brief Convert string from lower_case style to CamelCase
    \return string
  */
  {"CamelCaseToLowerCase", gmStringCamelCaseToLowerCase},
/*
