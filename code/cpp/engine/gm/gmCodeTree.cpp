    ...
    while(a_node != NULL)
    {
      ...
      if(a_node->m_type == CTNT_DECLARATION)
      {
        ...
        switch(a_node->m_subType)
        {
          ...
          case CTNDT_VARIABLE : fprintf(a_fp, "CTNDT_VARIABLE:%04d, type %ld" GM_NL, a_node->m_lineNumber, a_node->m_subTypeType); break;
...
gmCodeTreeNode * gmCodeTreeNode::Create(gmCodeTreeNodeType a_type, int a_subType, int a_lineNumber, intptr_t a_subTypeType)
{
...
static bool gmFold(float &a_r, float a_a, intptr_t a_op)
{
  switch(a_op)
  {
...
static bool gmFold(int &a_r, int a_a, intptr_t a_op)
{
  switch(a_op)
  {
    case CTNOT_UNARY_PLUS : a_r = a_a; break;
...
#include <math.h>
static bool gmFold(float &a_r, float a_a, float a_b, intptr_t a_op)
{
  switch(a_op)
  {
...
static bool gmFold(int &a_r, int a_a, int a_b, intptr_t a_op)
{
  switch(a_op)
  {
    case CTNOT_TIMES : a_r = a_a * a_b; break;
