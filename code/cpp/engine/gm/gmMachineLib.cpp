...
static int GM_CDECL gmTypeId(gmThread * a_thread) // return int, or null
{
  ...
  {
    a_thread->PushInt((int) a_thread->Param(0).m_type);
  }
  return GM_OK;
}
...
static int GM_CDECL gmSetThreadGroupActive(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(2);
  GM_CHECK_INT_PARAM(group, 0);
  GM_CHECK_INT_PARAM(active, 1);
  a_thread->GetMachine()->SetThreadGroupActive(group, active != 0);
  return GM_OK;
}
...
static int GM_CDECL gmSetStateOnThread(gmThread * a_thread) // (threadid, fp, params...) returns true or false.
{
  ...
  if(currentStateVariable->m_type == s_gmStateUserType)
  {
    ...
    if(currentState->m_setExitState)
    {
      gmThread * leave_thread = a_thread->GetMachine()->CreateThread(thisVar, gmVariable(GM_FUNCTION, currentState->m_setExitState->GetRef()));
      if(leave_thread)
      {
        leave_thread->Sys_Execute();
      }
    }
...
static int GM_CDECL gmTableFirst(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(1);
  GM_CHECK_TABLE_PARAM(table, 0);
  ...
  else
  {
	...
	a_thread->Push( node->m_value );
  }

  return GM_OK;
}

static int GM_CDECL gmTableAtIndex(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(2);
  GM_CHECK_TABLE_PARAM(table, 0);
  ...
  else
  {
	...
	int count = 0;
	for( ; node && (count<=index); node = table->GetNext(it), ++count )
	{
		if ( count == index ) 
		{
			a_thread->Push( node->m_value );
...
static int GM_CDECL gmTableGetSortedKeys(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(2);
  GM_CHECK_TABLE_PARAM(table, 0);
  GM_CHECK_TABLE_PARAM(keys, 1);

  gmVariable* results[1024] = { 0 }; 

  if ( table->Count() > 1024 )
  {
      a_thread->GetMachine()->GetLog().LogEntry("too many keys to sort");
      return GM_EXCEPTION;
  }

  gmSortTableKeysRaw( table, results );

  for ( int i = 0; i < table->Count(); ++i )
  {
      keys->Set( a_thread->GetMachine(), i, *results[i] );
  }

  a_thread->PushTable( keys );
  return GM_OK;
}
...
{ 
  ...
  */
  ...
  /*gm
    \function setThreadGroupActive
    \brief 
    \param int group
  */
  {"setThreadGroupActive", gmSetThreadGroupActive},
  ...
  */
  ...
  /*gm (FUNK)
    \function tableFirst
    \brief tableFirst will return first table item or null of DNE
    \param table
  */
  {"tableFirst", gmTableFirst},

  /*gm (FUNK)
    \function tableAtIndex
    \brief tableAtIndex will return item at index or null if DNE
    \param table
  */
  {"tableAtIndex", gmTableAtIndex},

  /*gm (FUNK)
    \function tableGetSortedKeys
    \brief tableGetSortedKeys will fill a passed table with keys in sorted order
    \param table
    \param keys
  */
  {"tableGetSortedKeys", gmTableGetSortedKeys},
