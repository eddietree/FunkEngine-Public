...
// ported from hidef
void gmTableObject::ClearTable(gmMachine * a_machine, bool a_disableWriteBarrier )
{
#if GM_USE_INCGC
	if (!a_disableWriteBarrier)
	{
		for ( int index = m_tableSize - 1; index >= 0; --index )
		{
			gmTableNode *node = &m_nodes[index];
			if(node->m_key.IsReference())
				a_machine->GetGC()->WriteBarrier((gmObject*)node->m_key.m_value.m_ref);
			if(node->m_value.IsReference())
				a_machine->GetGC()->WriteBarrier((gmObject*)node->m_value.m_value.m_ref);
		}
	}
#endif
...
/* original attempt for FunkEngine -- doesnt handle marking children for gc
void gmTableObject::ClearTable(gmMachine * a_machine)
{
	// already empty
	if (m_nodes == NULL)
		return;
