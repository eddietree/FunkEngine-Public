...
static int GM_CDECL gmfFileExists(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(1);
  GM_CHECK_STRING_PARAM(filename, 0);
  a_thread->PushInt( gmFileExists(filename) ? 1:0 );
  return GM_OK;
}
...
static int GM_CDECL gmfGetEnvironmentVariable(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(1);
  ...
  DWORD result = ::GetEnvironmentVariable(name, buffer, 1024);
...
static int GM_CDECL gmfGetUserName(gmThread * a_thread)
{
...
static int GM_CDECL gmfSaveTableToFile(gmThread * a_thread) // int, return char written, or NULL on error
{
	GM_CHECK_NUM_PARAMS(2);
...
static int GM_CDECL gmfLoadTableFromFile(gmThread * a_thread) // int, return char written, or NULL on error
{
	GM_CHECK_NUM_PARAMS(1);
...
static gmFunctionEntry s_systemLib[] = 
{
 ...
 /*gm
	 ...
	 \brief Load gm table from a file
	 \param string filename
	 \return table object
	 */
  ...
  */
  ...
  /*gm
    ...
    \param string format ("%A %d %B %Y, %I:%M:%S %p") is the format string to use.<BR>
            ...
            %p : Current locale痴 A.M./P.M. indicator for 12-hour clock<BR>
  ...
  */
  ...
  /*gm
    \function GetDirectoryList (FUNK)
    \brief GetDirectoryList will give a list of all directories in the given path
    \param string folder to search
    \return table of folder names
  */
  {"GetDirectoryList", gmfGetDirectoryList},

  {"GetEnvironmentVariable", gmfGetEnvironmentVariable},

  {"GetUserName", gmfGetUserName},
#endif // WIN32
};
