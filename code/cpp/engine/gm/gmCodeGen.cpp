        ...
        switch(a_node->m_subType)
        {
          case CTNET_OPERATION : 
          {
            switch(a_node->m_subTypeType)
            {
			  ...
			  case CTNOT_UNARY_ISNULL : res = GenExprOpUnary(a_node, a_byteCode); break;
              case CTNOT_UNARY_ISNOTNULL : res = GenExprOpUnary(a_node, a_byteCode); break;
...
bool gmCodeGenPrivate::GenExprOpUnary(const gmCodeTreeNode * a_node, gmByteCodeGen * a_byteCode)
{
  ...
  switch(a_node->m_subTypeType)
  {
	...
	case CTNOT_UNARY_ISNOTNULL :  return a_byteCode->Emit(BC_ISNOTNULL);
    default :
    {
