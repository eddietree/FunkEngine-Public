...
const char * gmVariable::AsString(gmMachine * a_machine, char * a_buffer, int a_len) const
{
  switch(m_type)
  {
    ...
    case GM_INT :
      ...
      break;
	case GM_V2 :
		_gmsnprintf(a_buffer, a_len, "v2(%f, %f)", m_value.m_v2.x, m_value.m_v2.y );
		break;
	case GM_V2I :
		_gmsnprintf(a_buffer, a_len, "v2i(%d, %d)", m_value.m_v2i.x, m_value.m_v2i.y );
		break;
	case GM_V3 :
		_gmsnprintf(a_buffer, a_len, "v3(%f, %f, %f)", m_value.m_v3.x, m_value.m_v3.y, m_value.m_v3.z );
		break;
    case GM_FLOAT :
    ...
    default:
      ...
      else
      {
		...
		_gmsnprintf(a_buffer, a_len, "%s:0x%llx", a_machine->GetTypeName(m_type), m_value.m_ref);
		#else
