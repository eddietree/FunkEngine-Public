...
void gmMachine::SetThreadGroupActive( int a_group, bool a_active )
{
	GM_ASSERT( a_group < GM_MACHINE_THREADGROUPS );

	m_activeThreadGroup[ a_group ] = a_active;
}
...
#if GMMACHINE_TRACK_THREAD_ALLOC_COUNTS
  static int frame_ = 0;
  frame_++;
  ClearAllocCounts();
#endif
...
// NOTE: would rather work priorities but for now just a simple group sorting
#define GMTHREAD_SORT_BY_GROUP
#ifdef GMTHREAD_SORT_BY_GROUP

  // Sort running threads

  static std::vector<gmThread*> sorted;

  sorted.clear();

  gmThread* iter = m_runningThreads.GetFirst();

  while ( m_runningThreads.IsValid( iter ) )
  {
	  sorted.push_back( iter );

	  iter = m_runningThreads.GetNext( iter );
  }

  struct { bool operator() ( gmThread* lhs, gmThread* rhs ) { return lhs->GetGroup() < rhs->GetGroup(); } } sorter;
  ...
  for ( ; a != b; ++a )
  {
	  m_runningThreads.InsertLast( *a );
  }

#endif // GMTHREAD_SORT_BY_GROUP
  ...
  for(it = m_runningThreads.GetFirst(); m_runningThreads.IsValid(it);)
  {
	...
	const int group = it->GetGroup();
	GM_ASSERT( group < GM_MACHINE_THREADGROUPS );
	const bool active = m_activeThreadGroup[ group ];
...
bool gmMachine::CollectGarbage(bool a_forceFullCollect)
{
  ...
  {
    //TRACE("************* COLLECTING GARBAGE ****************"GM_NL);
...
void gmMachine::ResetDefaultTypes()
{
  ...
  m_types[GM_V2].m_name = AllocPermanantStringObject("v2");
  m_types[GM_V2I].m_name = AllocPermanantStringObject("v2i");
  m_types[GM_V3].m_name = AllocPermanantStringObject("v3");
  ...
  gmInitBasicType(GM_V2, m_types[GM_V2].m_nativeOperators);
  gmInitBasicType(GM_V2I, m_types[GM_V2I].m_nativeOperators);
  gmInitBasicType(GM_V3, m_types[GM_V3].m_nativeOperators);
...
#if GMMACHINE_TRACK_THREAD_ALLOC_COUNTS
void gmMachine::AddCountObj(gmObject* a_obj)
{
	gmThread* thread = GetCurrentThread();
	// can be null during gmMachine init
	if (thread == NULL)
		return;

	const gmFunctionObject* func = thread->GetFunctionObject();
	m_allocCounts[func] += 1;
}

void gmMachine::ClearAllocCounts()
{
	m_allocCounts.clear();
}

std::map<const gmFunctionObject*, int>::const_iterator gmMachine::GetAllocCountIteratorBegin()
{
	return m_allocCounts.begin();
}

std::map<const gmFunctionObject*, int>::const_iterator gmMachine::GetAllocCountIteratorEnd()
{
	return m_allocCounts.end();
}
